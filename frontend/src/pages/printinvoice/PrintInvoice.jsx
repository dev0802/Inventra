...
export default function PrintInvoice() {
  ...
  const [generalInvoiceMode, setGeneralInvoiceMode] = useState(false);
  ...
  useEffect(() => {
    const fetchInitial = async () => {
      try {
        const goldRate = await getGoldRateByDateApi(todayDate());
        ...
        console.log("Date:", goldRate?.rate_date);
      } catch (error) {
        console.error("Gold rate not found for today");
      }
    };
    fetchInitial();
  }, []);
  ...
  console.log("General Invoice Mode: ", generalInvoiceMode);
                ...
                {rows.map((row) => (
                  <tr key={row.id}>
                    <td className="p-2 border border-gray-300">
                      <input
                        className="border rounded px-2 py-1 w-10 bg-white text-center focus:outline-none"
                        value={row.itemCode}
                        onChange={(e) =>
                    ...
                    <td className="p-2 border border-gray-300 ">
                      ...
                      >
                        <option value=""></option>
                        {row.itemDescription &&
                          !itemDescriptions.includes(row.itemDescription) && (
                            <option value={row.itemDescription}>
                              {row.itemDescription}
                            </option>
                          )}
                    ...
                    <td className="p-2 border border-gray-300 text-center">
                      <select
                        className="border rounded px-2 py-1 w-20 bg-white focus:outline-none"
                        value={row.unit}
                        onChange={(e) =>
                    ...
                    <td className="p-2 border border-gray-300 text-center">
                      <input
                        type="checkbox"
                        className="w-5 h-5 cursor-pointer accent-gray-600 focus:outline-none"
                    ...
                    <td className="p-2 border border-gray-300">
                      <input
                        className="border rounded px-2 py-1 w-24 bg-white text-center focus:outline-none"
